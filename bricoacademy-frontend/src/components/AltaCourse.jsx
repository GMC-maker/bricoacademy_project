...
export default function AltaCourse() {
	...
	useEffect(() => {
		async function fetchCreateCourse() {
			try {
				const payload = {
					...course,
					id_teacher:
				...
				const respuesta = await api.post("/courses/", payload);

				setDialogMessage(
					respuesta.mensaje || "Curso registrado correctamente",
				);
		...
		if (isUpdating) fetchCreateCourse();
	}, [isUpdating]);
